...
def test(net, all=False, stride=None, batch_size=None, window_size=None, device='cpu'):
    ...
    nstart = 0
    ntarg = nstart+npred
    #rand_idx = random.sample(range(0,samp_test.size()[0]),batch_size)
    ...
    for ib in range(0, batch_size):
        ...
        #acc[ib] = float( np.abs(inp-out).max() / np.abs(out).max() )
        inp_norm = np.exp(inp)
        out_norm = np.exp(out)
...
def main(save=False, pretrained=True, task='test'):
    ...
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, [500, 750, 900], gamma=0.5) # [500, 750, 900]
    ...
    if save:
        if task == 'test':
            np.savez(cfg.exp_dir + cfg.PRED_PATH + cfg.DATASET+
                 '_pred'+cfg.TEST_FILE[-6:-4]+cfg.LABELS[0].format(cfg.N_LAYERS, cfg.FIL_SIZE[0],cfg.DIL_M[0],int(10), 1000)+'n1', 
